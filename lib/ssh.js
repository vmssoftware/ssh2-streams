...
var readUInt32BE = require('./buffer-helpers_').readUInt32BE;
var writeUInt32BE = require('./buffer-helpers_').writeUInt32BE;
...
function onKEXDH_REPLY(self, info, verifiedHost) { // Client
  ...
  if (!hostPubKey.verify(outstate.exchangeHash, rawsig)) {
    ...
    if (self.config.skipSignatureVerification !== true) { // true === skip
      self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
      self.reset();
